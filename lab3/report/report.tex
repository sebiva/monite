...
The below example illustrates how the type signatures of an ExceptT outside of a
StateT allows for preservation of state when encountering an exception.

\begin{verbatim}
  M :: ExceptT e (StateT s m) r

  runExceptT M :: StateT s m (Either e r)
  runStateT (runExceptT M) :: s -> m (Either e r, s)

  runM :: M -> s -> m (Either e r, s)
  runM m = runStateT (runExceptT m)
