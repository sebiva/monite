...
it would be possible to improve the one of the mudules in the library, that is,
...
which instead of only looking in the directory from which the application is
executed , looks in a set of supplied directories. In our case, this is used to
...
functions. For a suggestion of such an interface (sadly undefined due to time
limitations) see the end of the file System/Console/Haskeline/Completion.hs.
...
work was required. We thought it would be interesting to see an actual
...
case a little cumbersome to use, and we have made some suggestions on how to
improve upon this with a compositional interface for completion functions (see
...
outside/inside of a StateT allows for preservation/non-preservation of state
...
\begin{verbatim}
  ...
  -- Here we see the error, result and state
  ...
  runExceptT (runStateT y s) :: m (Either e (r, s))

  -- Here we see either the error, or the (result, state)
  runM' :: Y -> s -> m (Either e (r, s))
...
Is also more focused as a scripting language rather than an interactive shell,
...
Haskeline is used to provide a rich command-line interface for the
...
have written our own BNFC grammars, from which BNFC then generates a lexer, and
a parser, which are used to parse the commands entered on the command-line in
Monite. The abstract syntax tree is then evaluated using our own own
interpreter.
...
Wraps within parentheses. These parentheses mean that what is inside it should be
...
\begin{verbatim}
  { $i : i <- [pwd, ls] } ; # Should print the output of first pwd, then ls
\end{verbatim}
